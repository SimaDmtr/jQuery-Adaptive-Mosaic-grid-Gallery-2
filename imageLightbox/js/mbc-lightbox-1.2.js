...
var ILB = (function(){
	var x = {
		...
		caption : {
			...
			on : function(){
				if ($('#imagelightbox-text').html() === '') x.caption.off();
		...
		create : function(classe){
			...
			$(function(){
				x.overlay.create();
				x.loader.create();
				x.caption.create(classe); //eventually pass string "visible" to have default caption always visible.
				x.closeButton.create();
			});
		},
		opt : {
			onStart: 	 function() { x.overlay.on(); x.closeButton.on();},
			onEnd:	 	 function() { x.overlay.off(); x.closeButton.off(); x.caption.off(); x.loader.off(); },
			onLoadStart: function() { x.loader.on(); x.caption.off(); },
			onLoadEnd:	 function() { x.loader.off(); x.caption.on(); }
		}
	};
	return x;
})();
...
function imageLightbox(_a,_b){
	...
	//or set default options
	else{
		ILB.create();
		this.setOptions(ILB.opt);
	}
	...
	this.setImage = function(){
		...
		tmpImage.onload = function(){
			...
			//create description
			$('#'+options.caption).html(targets[targetID].title);
		};
		//start loading image
	...
	this.addTrigger = function($elem,imgID){
	...
	this.addToGallery = function($elem){
		var o = {src: $elem.attr('href'), title: $elem.attr('title') || $elem.data('title') || " "}
		var id = targets.push(o) -1;
		this.addTrigger($elem,id);
	}
...
$.fn.imageLightbox = function(opt){
	return this.each(function(){
		var self = $(this);
		self[0].lightbox = new imageLightbox(opt);
		self.find('a').each(function(){
			self[0].lightbox.addToGallery($(this));
		});
	});
}